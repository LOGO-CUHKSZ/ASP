...
class CVRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1, prepare_dataset=None):
        if prepare_dataset is None:
            ...
            if self.problem_size == 10:
                demand_scaler = 20
            elif self.problem_size == 20:
                demand_scaler = 30
            elif self.problem_size in [30,40,50]:
                demand_scaler = 40
            elif self.problem_size in [60,70,80,90,100]:
                demand_scaler = 50
            else:
                raise NotImplementedError
            if len(prepare_dataset.shape)==3:
                node_demand = prepare_dataset[:, 1:, -1]
            else:
                node_demand = torch.randint(1, 10, size=(self.batch_size, self.problem_size), device=depot_xy.device) / float(demand_scaler)
