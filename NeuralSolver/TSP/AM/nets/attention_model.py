...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False):
        ...
        #     embeddings = self.adapter_after_embedder(embeddings)
...
class MixModel(AttentionModel):
    ...
    def forward(self, input, return_pi=False):
        ...
        for i in range(len(self.mix_prob)):
            mix_embeddings+=self.adapter_list[i](embeddings)*self.mix_prob[i]

        # embeddings_list = [self.adapter_list[i](embeddings)*self.mix_prob[i] for i in range(len(self.mix_prob))]
        #
        # embeddings = torch.stack(embeddings_list,dim=-1).sum(-1)

        _log_p, pi = self._inner(input, mix_embeddings)
