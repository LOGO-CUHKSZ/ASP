...
def main(size = 20):
  ...
  loc = np.random.uniform(size=(dataset_size, size, 2))
  prize_ = np.linalg.norm(depot[:, None, :] - loc, axis=-1)
  prize = (1 + (prize_ / prize_.max(axis=-1, keepdims=True) * 99).astype(int)) / 100.
  max_length = MAX_LENGTHS[size]
  data = DataProblem(depot[0].tolist(), loc[0].tolist(), prize[0].tolist(), max_length)
...
if __name__ == '__main__':
    ...
    prize = (1 + (prize_ / prize_.max(axis=-1, keepdims=True) * 99).astype(int)) / 100.
    max_length = MAX_LENGTHS[size]
    obj, route = solve_op_ortools(depot.tolist(), loc.tolist(), prize.tolist(), max_length, sec_local_search=0)