...
class Pre_VRPDataset(Dataset):

    def __init__(self, prepare_data):
        super(Pre_VRPDataset, self).__init__()
        num_samples = prepare_data.shape[0]
        if len(prepare_data.shape) == 3:
            self.data = [
                {
                    'loc': prepare_data[i,1:,:2],
                    'demand': prepare_data[i,1:,2],
                    'depot': prepare_data[i,0,:2]
                }
                for i in range(num_samples)
            ]
        else:
            size = prepare_data.shape[1] - 1
            CAPACITIES = {
                10: 20.,
                20: 30.,
                30: 30.,
