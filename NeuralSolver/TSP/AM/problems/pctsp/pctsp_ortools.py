...
class DataProblem():
  ...
  def __init__(self, depot, loc, prize, penalty, min_prize):
    ...
    self._min_prize = min(float_to_scaled_int(min_prize), sum(self._prizes))
...
def solve_pctsp_ortools(depot, loc, prize, penalty, min_prize, sec_local_search=0):
    ...
    distance_evaluator = routing.RegisterTransitCallback(distance_evaluator)
    routing.SetArcCostEvaluatorOfAllVehicles(distance_evaluator)

    # Add minimum total prize constraint
    prize_evaluator = CreatePrizeEvaluator(data, manager).prize_evaluator
    prize_evaluator = routing.RegisterTransitCallback(prize_evaluator)
...
if __name__=="__main__":
    ...
    prize = np.random.rand(size) * 4 / float(size)
    penalty_max = MAX_LENGTHS[size] * (penalty_factor) / float(size)
    penalty = np.random.rand(size) * penalty_max
    min_prize = min(sum(prize), 1.)
    print(solve_pctsp_ortools(depot.tolist(), loc.tolist(), prize.tolist(), penalty.tolist(), min_prize, sec_local_search=0))