...
def inherit_instance(data, penalty_factor=3):
    if data.shape[-1]==5:
        loc = data[1:,:2]
        depot = data[0,:2]
        penalty = data[1:,2]
        deterministic_prize = data[1:,3]
        stochastic_prize = data[1:,4]
...
class Pre_PCTSPDataset(Dataset):

    def __init__(self, prepare_data):
        super(Pre_PCTSPDataset, self).__init__()
        # Currently the distribution can only vary in the type of the prize
        num_samples = prepare_data.shape[0]

        self.data = [
            inherit_instance(prepare_data[i])
            for i in range(num_samples)
        ]
