...
def get_options(parser=None):
    ...
    parser.add_argument('--random_seed', type=int, default=1234, help='Random seed to use')
    ...
    # opts.solver_val_size = opts.val_size
    try:
        opts.n_encode_layers = opts.solver_n_encode_layers
    except:
        opts.n_encode_layers = 9
    try:
        if opts.train_from_scratch == True:
            opts.solver_epoch_size = int(opts.epoch_size/opts.num_batch)
            opts.solver_batch_size = opts.batch_size
